function isPalindrome(x: number): boolean {
    ...
    const log10: number = Math.floor(Math.log10(x));
    for (let i = log10; i > log10 / 2; i--) {
        const firstNumber: number = Math.floor(x / Math.pow(10, i)) % 10; // 1st loop iteration: 9229 => x / 10 ^ 3 => 9 % 10 => 9
        const lastNumber: number = Math.floor(x / Math.pow(10, log10 - i)) % 10; // 2nd loop iteration: 9229 => x / 10 ^ 1 => 922 % 10 => 2
